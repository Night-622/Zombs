...
const WEAPONS = {
  ...
  pickaxe: { name:"Pickaxe",  icon:"⛏",  type:"melee",  damage:10, range:50, arc:1.0, cooldown:460, cost:35, order:3,
  ...
  pistol:  { name:"Pistol",   icon:"🔫", type:"ranged", damage:11, cooldown:290, bulletSpeed:640, spread:0.05, ammoCost:1, cost:80,  order:5 },
  smg:     { name:"SMG",      icon:"💥", type:"ranged", damage:6,  cooldown:95,  bulletSpeed:680, spread:0.13, ammoCost:1, cost:180, order:6 },
  shotgun: { name:"Shotgun",  icon:"✹",  type:"ranged", damage:9,  pellets:6, cooldown:680, bulletSpeed:600, spread:0.38, ammoCost:3, cost:260, order:7 },
  rifle:   { name:"Rifle",    icon:"🎯", type:"ranged", damage:20, cooldown:240, bulletSpeed:760, spread:0.04, ammoCost:2, cost:400, order:8 },
  sniper:  { name:"Sniper",   icon:"🔭", type:"ranged", damage:65, cooldown:1150,bulletSpeed:1100,spread:0.008,ammoCost:4, cost:620, order:9 },
  rocket:  { name:"Rocket",   icon:"🚀", type:"ranged", damage:75, splash:85, cooldown:1450, bulletSpeed:480, spread:0.02, ammoCost:8, cost:1000, order:10 },
};
const WEAPON_ORDER = Object.keys(WEAPONS).sort((a,b)=>WEAPONS[a].order-WEAPONS[b].order);

/* ============================== DATA: ENEMIES ============================== */
const ENEMY_TYPES = {
  zombie:  { name:"Zombie",   hp:20,  speed:52,  damage:9,  gold:5,  radius:14, color:"#5fae3d", minWave:1 },
  runner:  { name:"Runner",   hp:10,  speed:108, damage:6,  gold:7,  radius:11, color:"#e0c93f", minWave:2 },
  brute:   { name:"Brute",    hp:150, speed:34,  damage:24, gold:18, radius:21, color:"#8a5a3d", minWave:3 },
  spitter: { name:"Spitter",  hp:38,  speed:46,  damage:8,  gold:11, radius:13, color:"#9b5fd6", minWave:4,
             ranged:true, projSpeed:340, atkRange:230, atkCooldown:1700 },
  boomer:  { name:"Boomer",   hp:22,  speed:58,  damage:2,  gold:9,  radius:15, color:"#e2574c", minWave:3,
             explode:true, explodeDamage:38, explodeRadius:80 },
  boss:    { name:"Zomb King",hp:1900,speed:26,  damage:46, gold:350,radius:42, color:"#7a1015", minWave:5, boss:true },
};
...
/* ============================== DATA: BUILDINGS ============================== */
const BUILDING_TYPES = {
  woodWall:  { name:"Wood Wall",  icon:"▦", hp:90,  cost:{wood:10},            size:34, color:"#8d6e4a", upgradeTo:"stoneWall" },
  stoneWall: { name:"Stone Wall", icon:"▦", hp:220, cost:{stone:16},           size:34, color:"#9aa0a6", upgradeTo:"metalWall" },
  metalWall: { name:"Metal Wall", icon:"▦", hp:480, cost:{metal:18},           size:34, color:"#6f8493" },
  spike:     { name:"Spike Trap", icon:"✳",  hp:65,  cost:{wood:8,stone:5},    size:28, color:"#b1443b", damage:16, trap:true },
  turret:    { name:"Auto Turret",icon:"⛭",  hp:160, cost:{metal:32,gold:28},  size:32, color:"#e0a831", turret:true, range:270, damage:13, cooldown:520, bulletSpeed:700 },
  mortar:    { name:"Mortar",     icon:"◉",  hp:220, cost:{metal:65,gold:80},  size:36, color:"#7a5a3a", turret:true, range:360, damage:48, splash:65, cooldown:1650, bulletSpeed:420 },
  campfire:  { name:"Campfire",   icon:"🔥", hp:60,  cost:{wood:18,stone:10},  size:26, color:"#e0763a", heal:true, healRange:120, healAmount:4, healInterval:1000 },
};
const BUILDING_ORDER = ["woodWall","stoneWall","metalWall","spike","turret","mortar","campfire"];

/* ============================== DATA: SURVIVOR UPGRADES ============================== */
const UPGRADES = {
  health: { name:"Max Health", icon:"❤", base:40, growth:1.45, max:8, effect:"+20 max HP", per:20 },
  speed:  { name:"Move Speed", icon:"👟", base:50, growth:1.5,  max:6, effect:"+6% speed",  per:0.06 },
  armor:  { name:"Armor",      icon:"🛡", base:60, growth:1.5,  max:6, effect:"-5% dmg taken", per:0.05 },
  harvest:{ name:"Harvesting", icon:"💪", base:45, growth:1.45, max:5, effect:"+15% harvest",per:0.15 },
};
...
function renderCraftTab() {
  ...
  note.textContent = "Arrows are fletched from wood, metal, and leaf.";
  el.appendChild(note);
  const recipes = [ {amt:8,wood:8,metal:8,leaf:8}, {amt:20,wood:18,metal:18,leaf:18}, {amt:50,wood:40,metal:40,leaf:40} ];
  recipes.forEach(r => {
    const cost = {wood:r.wood, metal:r.metal, leaf:r.leaf};
    const row = document.createElement("div"); row.className="shop-item";
    row.innerHTML = `<div class="si-info"><div class="si-name">🏹 Craft ${r.amt} Arrows</div><div class="si-cost">${r.wood} wood, ${r.metal} metal, ${r.leaf} leaf</div></div>
...
})();  axe:     { name:"Axe",      icon:"🪓", type:"melee",  damage:16, range:50, arc:1.0, cooldown:480, cost:60,   order:2, harvest:{tree:11,rock:3, ore:2} },
  pickaxe: { name:"Pickaxe",  icon:"⛏",  type:"melee",  damage:11, range:50, arc:1.0, cooldown:460, cost:60,   order:3, harvest:{tree:2, rock:11,ore:9} },
  pistol:  { name:"Pistol",   icon:"🔫", type:"ranged", damage:11, cooldown:290, bulletSpeed:640, spread:0.05, ammoCost:1, cost:150,  order:4 },
  smg:     { name:"SMG",      icon:"💥", type:"ranged", damage:6,  cooldown:95,  bulletSpeed:680, spread:0.13, ammoCost:1, cost:320,  order:5 },
  shotgun: { name:"Shotgun",  icon:"✹",  type:"ranged", damage:9,  pellets:6, cooldown:680, bulletSpeed:600, spread:0.38, ammoCost:3, cost:480, order:6 },
  rifle:   { name:"Rifle",    icon:"🎯", type:"ranged", damage:20, cooldown:240, bulletSpeed:760, spread:0.04, ammoCost:2, cost:720,  order:7 },
  sniper:  { name:"Sniper",   icon:"🏹", type:"ranged", damage:65, cooldown:1150,bulletSpeed:1100,spread:0.008,ammoCost:4, cost:1100, order:8 },
  rocket:  { name:"Rocket",   icon:"🚀", type:"ranged", damage:75, splash:85, cooldown:1450, bulletSpeed:480, spread:0.02, ammoCost:8, cost:1800, order:9 },
};
const WEAPON_ORDER = Object.keys(WEAPONS).sort((a,b)=>WEAPONS[a].order-WEAPONS[b].order);

/* ============================== DATA: ENEMIES ============================== */
const ENEMY_TYPES = {
  zombie:  { name:"Zombie",   hp:32,  speed:52,  damage:9,  gold:5,  radius:14, color:"#5fae3d", minWave:1 },
  runner:  { name:"Runner",   hp:16,  speed:108, damage:6,  gold:7,  radius:11, color:"#e0c93f", minWave:2 },
  brute:   { name:"Brute",    hp:150, speed:34,  damage:24, gold:18, radius:21, color:"#8a5a3d", minWave:3 },
  spitter: { name:"Spitter",  hp:38,  speed:46,  damage:8,  gold:11, radius:13, color:"#9b5fd6", minWave:4,
             ranged:true, projSpeed:340, atkRange:230, atkCooldown:1700 },
  boomer:  { name:"Boomer",   hp:22,  speed:58,  damage:2,  gold:9,  radius:15, color:"#e2574c", minWave:3,
...
/* ============================== DATA: BUILDINGS ============================== */
const BUILDING_TYPES = {
  woodWall:  { name:"Wood Wall",  icon:"▦", hp:90,  cost:{wood:20},            size:34, color:"#8d6e4a", upgradeTo:"stoneWall" },
  stoneWall: { name:"Stone Wall", icon:"▦", hp:220, cost:{stone:30},           size:34, color:"#9aa0a6", upgradeTo:"metalWall" },
  metalWall: { name:"Metal Wall", icon:"▦", hp:480, cost:{metal:35},           size:34, color:"#6f8493" },
  spike:     { name:"Spike Trap", icon:"✳",  hp:65,  cost:{wood:15,stone:10},  size:28, color:"#b1443b", damage:16, trap:true },
  turret:    { name:"Auto Turret",icon:"⛭",  hp:160, cost:{metal:60,gold:60},  size:32, color:"#e0a831", turret:true, range:270, damage:13, cooldown:520, bulletSpeed:700 },
  mortar:    { name:"Mortar",     icon:"◉",  hp:220, cost:{metal:130,gold:170},size:36, color:"#7a5a3a", turret:true, range:360, damage:48, splash:65, cooldown:1650, bulletSpeed:420 },
  campfire:  { name:"Campfire",   icon:"🔥", hp:60,  cost:{wood:40,stone:20},  size:26, color:"#e0763a", heal:true, healRange:120, healAmount:4, healInterval:1000 },
};
const BUILDING_ORDER = ["woodWall","stoneWall","metalWall","spike","turret","mortar","campfire"];

/* ============================== DATA: SURVIVOR UPGRADES ============================== */
const UPGRADES = {
  health: { name:"Max Health", icon:"❤", base:70, growth:1.55, max:8, effect:"+20 max HP", per:20 },
  speed:  { name:"Move Speed", icon:"👟", base:90, growth:1.6,  max:6, effect:"+6% speed",  per:0.06 },
  armor:  { name:"Armor",      icon:"🛡", base:110,growth:1.65, max:6, effect:"-5% dmg taken", per:0.05 },
  harvest:{ name:"Harvesting", icon:"💪", base:80, growth:1.55, max:5, effect:"+15% harvest",per:0.15 },
};
...
function newPlayer() {
  return {
    ...
    baseSpeed: 190,
    facing: 0,
    wood: 40, stone: 20, metal: 10, gold: 0, ammo: 20,
...
function serialize() {
  ...
  return {
    ...
    player: {
      x:p.x, y:p.y, hp:p.hp, upgrades:p.upgrades,
      wood:p.wood, stone:p.stone, metal:p.metal, gold:p.gold, ammo:p.ammo,
      weapons:p.weapons, currentWeapon:p.currentWeapon,
    },
    buildings: G.buildings.map(b => ({ type:b.type, x:b.x, y:b.y, hp:b.hp })),
  };
...
function harvestNode(node) {
  const w = WEAPONS[G.player.currentWeapon];
  const table = w.harvest || {tree:2,rock:2,ore:2};
  const amt = Math.round((table[node.type]||1) * playerHarvestMult(G.player));
  const gained = Math.min(amt, node.hp);
  node.hp -= gained;
  const key = node.type==="tree"?"wood":node.type==="rock"?"stone":"metal";
  G.player[key] += gained;
  spawnFloater(node.x, node.y-14, `+${gained} ${key}`, "#c5e1a5");
  spawnParticles(node.x, node.y, node.type==="tree"?"#7cb342":node.type==="rock"?"#b0bec5":"#90a4ae", 5);
  if (node.hp <= 0) node.respawnAt = G.time + rand(30000, 55000);
}
function interact() {
...
function render() {
  ...
  if (G.player) {
    ...
    ctx.rotate(p.facing);
    ctx.fillStyle="#3f7a2b"; ctx.beginPath(); ctx.arc(0,0,CONFIG.PLAYER_RADIUS,0,7); ctx.fill();
    ctx.fillStyle="#c5e1a5"; ctx.fillRect(6,-4,16,8); // weapon nub
    ctx.restore();
  }
...
function renderCraftTab() {
  const el = document.getElementById("tabCraft"); el.innerHTML="";
  const recipes = [ {amt:20,metal:8}, {amt:60,metal:20}, {amt:150,metal:45} ];
  recipes.forEach(r => {
    const row = document.createElement("div"); row.className="shop-item";
    row.innerHTML = `<div class="si-info"><div class="si-name">🔩 Craft ${r.amt} Ammo</div><div class="si-cost">${r.metal} metal</div></div>
      <button class="si-btn">CRAFT</button>`;
    row.querySelector("button").onclick = () => {
      if (G.player.metal<r.metal) { toast("Not enough metal.", true); return; }
      G.player.metal -= r.metal; G.player.ammo += r.amt; updateUI(); toast(`Crafted ${r.amt} ammo.`);
    };
    el.appendChild(row);
  });
}
